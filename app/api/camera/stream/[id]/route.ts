...
export async function GET(
  ...
  try {
    ...
    const camera = await convex.query(api.cameras.getCameraById, {
      cameraId: id as any,
    });
    ...
    if (
      ...
      const cameraNumberMatch = camera.name.match(
        /(\d+)(?:\s*-\s*Pinto Los Pellines)?$/
      );
      const cameraNumber = cameraNumberMatch
        ? parseInt(cameraNumberMatch[1])
        : 1;

      // Get the appropriate UID based on camera number
      const uidKey =
        cameraNumber === 1 ? 'LSVISION_UID' : `LSVISION_UID_${cameraNumber}`;
      const lsvisionUid = process.env[uidKey];

      if (!lsvisionUid) {
        return new NextResponse(
          `LS Vision O-Kamm UID not configured for camera ${cameraNumber} (${uidKey})`,
          {
            status: 503,
            headers: {
    ...
    // Provide more detailed error responses based on error type
    if (error instanceof Error) {
      ...
      if (error.message.includes('network') || error.message.includes('timeout')) {
        return new NextResponse(
          JSON.stringify({
            error: 'Network error',
            message: 'Unable to connect to camera service',
          }),
          {
            status: 503,
            headers: { 'Content-Type': 'application/json' },
    ...
    return new NextResponse(
      JSON.stringify({
        ...
        message: 'An unexpected error occurred while accessing the camera stream',
      }),
      {
        status: 500,
        headers: { 'Content-Type': 'application/json' },
