...
function AdminCalendarContent() {
  ...
  const [filter, setFilter] = useState<'all' | 'upcoming' | 'past' | 'today'>(
    'all'
  );
  ...
  const filteredEvents = allEvents.filter((event: any) => {
    ...
    const matchesCategory =
      categoryFilter === 'all' || event.categoryId === categoryFilter;
  ...
  const handleDeleteCategory = async (categoryId: string) => {
    if (
      confirm(
        '¿Estás seguro de que quieres eliminar esta categoría? Los eventos asociados pueden verse afectados.'
      )
    ) {
      try {
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-green-600">
                  {loading
                    ? '...'
                    : allEvents.filter(
                        (e: any) => getEventStatus(e) === 'upcoming'
                      ).length}
                </p>
              </div>
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-blue-600">
                  {loading
                    ? '...'
                    : allEvents.filter(
                        (e: any) => getEventStatus(e) === 'current'
                      ).length}
                </p>
              </div>
