...
import { AlertTriangle, Edit, Eye, Filter, Plus, Trash2, Wifi, WifiOff, Settings } from 'lucide-react'
...
const statusConfig = {
  online: { color: 'border-green-500 bg-green-50 dark:bg-green-950/20', icon: Wifi, badge: 'En línea' },
  offline: { color: 'border-red-500 bg-red-50 dark:bg-red-950/20', icon: WifiOff, badge: 'Fuera de línea' },
  inactive: { color: 'border-gray-500 bg-gray-50 dark:bg-gray-950/20', icon: Settings, badge: 'Inactiva' }
}
...
function AdminCamerasContent() {
  ...
  const filteredCameras = allCameras.filter((camera: any) => {
    ...
    const matchesOnlineStatus = statusFilter === 'all' || cameraStatus === statusFilter
  ...
  const handleDeleteCamera = async (cameraId: string) => {
    if (confirm('¿Estás seguro de que quieres eliminar esta cámara? Esta acción no se puede deshacer.')) {
      try {
    ...
    >
      <div className='space-y-6'>
        ...
        >
          ...
          <Button onClick={handleCreateCamera} className='flex items-center space-x-2'>
          ...
          <Card>
            <CardContent className='p-6'>
              <div className='flex items-center justify-between'>
                <div>
                  <p className='text-sm font-medium text-gray-600 dark:text-gray-400'>Total Cámaras</p>
                  <p className='text-2xl font-bold'>{loading ? '...' : allCameras.length}</p>
                </div>
              ...
              <div className='flex items-center justify-between'>
                <div>
                  ...
                  <p className='text-2xl font-bold text-green-600'>
                    {loading ? '...' : allCameras.filter((c: any) => c.isOnline && c.isActive).length}
                  </p>
                </div>
              ...
              <div className='flex items-center justify-between'>
                <div>
                  ...
                  <p className='text-2xl font-bold text-gray-600'>
                    {loading ? '...' : allCameras.filter((c: any) => !c.isActive).length}
                  </p>
                </div>
        ...
        >
          {loading ? (
            ...
            <div className='text-center py-12'>
              ...
              <h3 className='text-xl font-semibold mb-2'>No se encontraron cámaras</h3>
                      ...
                      <CardContent>
                        <div className='space-y-2 mb-4'>
                          <div className='flex justify-between text-sm'>
                            ...
                            <span>{camera.location || 'No especificada'}</span>
                          </div>
                          <div className='flex justify-between text-sm'>
                            <span className='text-gray-600 dark:text-gray-400'>Última vez visto:</span>
              ...
              {selectedCamera && (
                <div className='space-y-4'>
                  ...
                  <div>
                    ...
                    <p className='font-mono text-sm break-all'>{selectedCamera.streamUrl}</p>
                  </div>
                </div>
              )}
            </div>
