...
import { AlertTriangle, Download, Edit, Eye, FileText, Filter, Plus, Trash2 } from 'lucide-react'
...
interface EmergencyProtocol {
  ...
  title: string
  description: string
  category: 'fire' | 'medical' | 'police' | 'natural_disaster' | 'security' | 'evacuation' | 'general'
...
const priorityConfig = {
  ...
  medium: { color: 'border-yellow-500 bg-yellow-50 dark:bg-yellow-950/20', icon: FileText, badge: 'Importante' },
  low: { color: 'border-blue-500 bg-blue-50 dark:bg-blue-950/20', icon: FileText, badge: 'Información' }
}
...
const categoryConfig = {
  ...
  natural_disaster: { emoji: '🌪️', color: 'text-orange-600', label: 'Desastre Natural' },
...
function AdminEmergencyProtocolsContent() {
  const { user } = useUser()
  const allProtocols = useQuery(api.emergency_protocols.getAllEmergencyProtocols) || []
  const deleteProtocol = useMutation(api.emergency_protocols.deleteEmergencyProtocol)
  ...
  const handleDeleteProtocol = async (protocolId: string) => {
    if (confirm('¿Estás seguro de que quieres eliminar este protocolo de emergencia? Esta acción no se puede deshacer.')) {
      try {
        ...
        >
          <div>
            ...
            <p className='text-gray-600 dark:text-gray-400 mt-1'>
              Administra todos los protocolos de emergencia y respuesta para la comunidad
            </p>
          </div>
          <Button onClick={handleCreateProtocol} className='flex items-center space-x-2'>
          ...
          <Card>
            ...
            <CardContent>
              <div className='text-2xl font-bold text-red-600'>
                {allProtocols.filter((p: any) => p.priority === 'high' || p.priority === 'critical').length}
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className='text-2xl font-bold text-blue-600'>
                {allProtocols.reduce((acc: number, p: any) => acc + p.downloadCount, 0)}
              </div>
            </CardContent>
          </Card>
        </motion.div>
          ...
          {loading ? (
            ...
            <div className='text-center py-12'>
              ...
              <p className='text-gray-600 dark:text-gray-400 mb-4'>
                {filter === 'all' ? 'Crea tu primer protocolo de emergencia para comenzar.' : 'No hay protocolos que coincidan con los filtros seleccionados.'}
              </p>
              {filter === 'all' && (
            ...
            <div className='space-y-4'>
              {filteredProtocols.map((protocol: any, index: number) => {
                const priority = priorityConfig[protocol.priority as keyof typeof priorityConfig]
                const category = categoryConfig[protocol.category as keyof typeof categoryConfig]
                      ...
                      <CardHeader className='pb-3'>
                        <div className='flex items-start justify-between'>
                          <div className='flex items-center space-x-3'>
                            <PriorityIcon className={`w-6 h-6 ${priority.color.includes('red') ? 'text-red-600' : priority.color.includes('yellow') ? 'text-yellow-600' : 'text-blue-600'}`} />
                            <div>
                          ...
                          <div className='flex items-center space-x-2'>
                            ...
                            {protocol.priority === 'high' || protocol.priority === 'critical' ? (
                      ...
                      <CardContent className='pt-0'>
                        <div className='flex items-center justify-between'>
                          <div className='flex items-center space-x-6 text-sm text-gray-600 dark:text-gray-400'>
                            ...
                            <span className='flex items-center space-x-1'>
                              <span>Contactos:</span>
                              <span className='font-medium'>{protocol.emergencyContacts.length}</span>
                            </span>
                            <span className='flex items-center space-x-1'>
                              <span>Pasos:</span>
                          ...
                          <div className='flex items-center space-x-2'>
                            <Button
                              variant='outline'
                              size='sm'
                              onClick={() => handleViewProtocol(protocol as any)}
                            >
                            ...
                            <Button
                              variant='outline'
                              size='sm'
                              onClick={() => handleEditProtocol(protocol as any)}
                            >
      ...
      <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
        <DialogContent className='max-w-4xl max-h-[90vh] overflow-y-auto'>
          <DialogHeader>
            ...
            <DialogDescription>
              Crea un nuevo protocolo de emergencia con contactos y instrucciones detalladas.
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={isViewDialogOpen} onOpenChange={setIsViewDialogOpen}>
        <DialogContent className='max-w-4xl max-h-[90vh] overflow-y-auto'>
          ...
          {selectedProtocol && (
            <div className='space-y-6'>
              <div>
                <h3 className='text-2xl font-semibold'>{selectedProtocol.title}</h3>
                <p className='text-gray-600 dark:text-gray-400 mt-2'>{selectedProtocol.description}</p>
                <div className='flex items-center space-x-2 mt-4'>
                  <Badge variant='outline'>
                    {categoryConfig[selectedProtocol.category].label}
                  </Badge>
                  <Badge variant={selectedProtocol.priority === 'high' || selectedProtocol.priority === 'critical' ? 'destructive' : 'secondary'}>
                    {priorityConfig[selectedProtocol.priority].badge}
                  </Badge>
                  <Badge variant={selectedProtocol.isActive ? 'default' : 'secondary'}>
              ...
              {/* Emergency Contacts */}
              <div>
                <h4 className='text-lg font-semibold mb-4'>Contactos de Emergencia</h4>
              ...
              {/* Steps */}
              <div>
                <h4 className='text-lg font-semibold mb-4'>Instrucciones Paso a Paso</h4>
              ...
              <div className='flex items-center justify-between text-sm text-gray-600 dark:text-gray-400 pt-4 border-t'>
                <div className='flex items-center space-x-4'>
                  ...
                  <span>Creado: {new Date(selectedProtocol.createdAt).toLocaleDateString('es-CL')}</span>
                  <span>Actualizado: {new Date(selectedProtocol.updatedAt).toLocaleDateString('es-CL')}</span>
                </div>
                {selectedProtocol.pdfUrl && (
                  <Button variant='outline' size='sm' asChild>
                    <a href={selectedProtocol.pdfUrl} target='_blank' rel='noopener noreferrer'>
