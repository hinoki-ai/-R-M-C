...
function AdminProjectsContent() {
  const { user } = useUser();
  const allProjects =
    useQuery(api.community_projects.getAllCommunityProjects, {}) || [];
  const deleteProject = useMutation(
    api.community_projects.deleteCommunityProject
  );
  ...
  const handleDeleteProject = async (projectId: string) => {
    if (
      !confirm(
        '¿Estás seguro de que quieres eliminar este proyecto comunitario? Esta acción también eliminará todas las contribuciones asociadas y no se puede deshacer.'
      )
    ) {
      return;
    }
          ...
          {loading ? (
            ...
            filteredProjects.map((project: CommunityProject) => {
              ...
              return (
                ...
                >
                  <Card
                    className={`border-l-4 ${statusConfig[project.status].color}`}
                  >
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            {project.isPublic && (
                              <Badge
                                variant="outline"
                                className="text-xs border-green-500 bg-green-50 dark:bg-green-950/20"
                              >
                                Público
                              </Badge>
                            )}
                          ...
                          <CardDescription className="flex items-center gap-1 mt-1">
                            ...
                            {formatCurrency(project.raised)} de{' '}
                            {formatCurrency(project.goal)} recaudado
                          </CardDescription>
                        </div>
                    ...
                    <CardContent>
                      ...
                      <div className="flex items-center justify-between text-xs text-gray-500">
                        <div className="flex gap-4">
                          ...
                          >
                            {daysUntilDeadline < 0
                              ? `${Math.abs(daysUntilDeadline)} días atrasado`
                              : daysUntilDeadline === 0
                                ? '¡Hoy!'
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600">
                      ...
                      {categoryConfig[selectedProject.category].label}
                    </p>
                  </div>
                  <div>
                    <label className="text-sm font-medium">Estado</label>
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600">
                      {selectedProject.isPublic ? 'Sí' : 'No'}
                    </p>
                  </div>
                </div>
