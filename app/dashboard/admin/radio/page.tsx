...
import { AlertTriangle, Edit, Eye, Filter, Globe, Music, Plus, Radio, Trash2, Wifi, WifiOff } from 'lucide-react'
...
function AdminRadioContent() {
  ...
  const filteredStations = allStations.filter((station: any) => {
    ...
    const matchesCategory = categoryFilter === 'all' || station.category === categoryFilter
  ...
  const handleDeleteStation = async (stationId: string) => {
    if (confirm('¿Estás seguro de que quieres eliminar esta estación de radio? Esta acción no se puede deshacer.')) {
      try {
        ...
        alert('Error al eliminar la estación de radio. Por favor, inténtalo de nuevo.')
      }
    }
  }
      ...
      <div className='space-y-6'>
        ...
        >
          <div>
            ...
            <p className='text-gray-600 dark:text-gray-400 mt-1'>
              Administra todas las estaciones de radio y configuraciones de streaming
            </p>
          </div>
          <Button onClick={handleCreateStation} className='flex items-center space-x-2'>
        ...
        >
          <div className='flex items-center space-x-4 flex-wrap gap-2'>
            ...
            <Select value={onlineFilter} onValueChange={(value: any) => setOnlineFilter(value)}>
              <SelectTrigger className='w-32'>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
        ...
        >
          ...
          <Card>
            <CardHeader className='flex flex-row items-center justify-between space-y-0 pb-2'>
              <CardTitle className='text-sm font-medium'>Calidad Alta</CardTitle>
          ...
          {loading ? (
            ...
            <div className='text-center py-12'>
              ...
              <p className='text-gray-600 dark:text-gray-400 mb-4'>
                {filter === 'all' ? 'Crea tu primera estación de radio para comenzar.' : 'No hay estaciones que coincidan con los filtros seleccionados.'}
              </p>
              {filter === 'all' && (
            ...
            <div className='space-y-4'>
              {filteredStations.map((station: any, index: number) => {
                const category = categoryConfig[station.category as keyof typeof categoryConfig]
                const quality = qualityConfig[station.quality as keyof typeof qualityConfig]
                ...
                return (
                  ...
                  >
                    <Card className={`border-l-4 ${station.isOnline ? 'border-l-green-500' : 'border-l-red-500'} hover:shadow-lg transition-shadow`}>
                          ...
                          <div className='flex items-center space-x-3'>
                            ...
                            <div>
                              ...
                              <CardDescription className='flex items-center space-x-2'>
                                <span>{station.frequency || 'Sin frecuencia'}</span>
                                <span>•</span>
                          ...
                          <div className='flex items-center space-x-4 text-sm text-gray-600 dark:text-gray-400'>
                            ...
                            {station.lastChecked && (
                              <span className='flex items-center space-x-1'>
                                ...
                                <span>{new Date(station.lastChecked).toLocaleDateString('es-CL')}</span>
                              </span>
                            )}
      ...
      <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
        <DialogContent className='max-w-2xl'>
          <DialogHeader>
            ...
            <DialogDescription>
              Crea una nueva estación de radio con su configuración de streaming.
            </DialogDescription>
          </DialogHeader>
          <RadioForm
            ...
            <div className='space-y-4'>
              <div className='flex items-center space-x-4'>
                ...
                <div>
                  ...
                  <p className='text-gray-600 dark:text-gray-400'>{selectedStation.frequency || 'Sin frecuencia'}</p>
                </div>
              </div>
              ...
              <div className='grid grid-cols-2 gap-4'>
                <div>
                  ...
                  <p className='text-gray-600 dark:text-gray-400'>{categoryConfig[selectedStation.category].label}</p>
                </div>
                <div>
                  <label className='text-sm font-medium'>Región</label>
                  <p className='text-gray-600 dark:text-gray-400'>{selectedStation.region}</p>
                </div>
                <div>
                  <label className='text-sm font-medium'>Calidad</label>
                  <p className='text-gray-600 dark:text-gray-400'>{qualityConfig[selectedStation.quality].label}</p>
                </div>
                <div>
                  ...
                  <div className='flex items-center space-x-2'>
                    ...
                    <Badge variant={selectedStation.isOnline ? 'default' : 'destructive'}>
                      {selectedStation.isOnline ? 'En línea' : 'Fuera de línea'}
                    </Badge>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <p className='text-gray-600 dark:text-gray-400 break-all'>{selectedStation.streamUrl}</p>
              </div>

              {selectedStation.backupStreamUrl && (
                <div>
                  <label className='text-sm font-medium'>URL de Backup</label>
                  <p className='text-gray-600 dark:text-gray-400 break-all'>{selectedStation.backupStreamUrl}</p>
                </div>
              )}
              ...
              {selectedStation.description && (
                <div>
                  ...
                  <p className='text-gray-600 dark:text-gray-400'>{selectedStation.description}</p>
                </div>
              )}

              <div className='flex items-center justify-between text-sm text-gray-600 dark:text-gray-400'>
                <div className='flex items-center space-x-4'>
                  <span>Creada: {new Date(selectedStation.createdAt).toLocaleDateString('es-CL')}</span>
                  <span>Actualizada: {new Date(selectedStation.updatedAt).toLocaleDateString('es-CL')}</span>
                </div>
              </div>
            </div>
          )}
