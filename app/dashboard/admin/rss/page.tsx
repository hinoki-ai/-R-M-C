...
function AdminRssContent() {
  ...
  const filteredFeeds = allFeeds.filter((feed: any) => {
    ...
    const matchesRegion =
      regionFilter === 'all' || feed.region === regionFilter;
  ...
  const handleDeleteFeed = async (feedId: string) => {
    if (
      confirm(
        '¿Estás seguro de que quieres eliminar este feed RSS? Esta acción no se puede deshacer y eliminará todos los artículos asociados.'
      )
    ) {
      try {
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-4">
              {filter === 'all'
                ? 'Crea tu primer feed RSS para comenzar.'
                : 'No hay feeds que coincidan con los filtros seleccionados.'}
            </p>
            {filter === 'all' && (
          ...
          <div className="space-y-4">
            {filteredFeeds.map((feed: any, index: number) => {
              const category =
                categoryConfig[feed.category as keyof typeof categoryConfig];
            ...
            <div className="space-y-4">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className="text-gray-600 dark:text-gray-400">
                    {selectedFeed.region} •{' '}
                    {categoryConfig[selectedFeed.category].label}
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div className="flex items-center space-x-2">
                    <Badge
                      variant={selectedFeed.isActive ? 'default' : 'secondary'}
                    >
                ...
                <div>
                  ...
                  <p className="text-gray-600 dark:text-gray-400">
                    Cada {selectedFeed.fetchInterval} minutos
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between text-sm text-gray-600 dark:text-gray-400">
                <div className="flex items-center space-x-4">
                  <span>
                    Creado:{' '}
                    {new Date(selectedFeed.createdAt).toLocaleDateString(
                      'es-CL'
                    )}
                  </span>
                  <span>
                    Actualizado:{' '}
                    {new Date(selectedFeed.updatedAt).toLocaleDateString(
                      'es-CL'
                    )}
                  </span>
                  {selectedFeed.lastFetched && (
                    <span>
                      Último fetch:{' '}
                      {new Date(selectedFeed.lastFetched).toLocaleDateString(
                        'es-CL'
                      )}
                    </span>
                  )}
