...
const useParticipationData = () => {
  ...
  return months.map(({ month, monthNum, year }) => {
    const monthEvents = events.filter((event: any) => {
      ...
      return (
        eventDate.getMonth() === monthNum && eventDate.getFullYear() === year
      );
    });
    ...
    const monthAnnouncements = announcements.filter((announcement: any) => {
      ...
      return (
        announcementDate.getMonth() === monthNum &&
        announcementDate.getFullYear() === year
      );
    });

    // Calculate attendance from events (if available)
    const totalAttendance = monthEvents.reduce(
      (sum: number, event: any) => sum + (event.attendance || 0),
      0
    );
...
const useMaintenanceData = () => {
  ...
  const maintenanceRequests =
    useQuery(api.community.getMaintenanceRequests) || [];
  ...
  return weeks.map((week, index) => {
    ...
    const completedRequests = weekRequests.filter(
...
const useEngagementData = () => {
  ...
  return days.map((dia, index) => {
    ...
    const dayData = engagementData.find((d: any) => d.dayOfWeek === index) || {
      views: 0,
      interactions: 0,
    };
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Indicadores clave de participación y actividad comunitaria (datos
            reales - muestra 0 si no hay datos)
          </p>
        </div>
      </div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Participación en eventos comunitarios últimos 6 meses (datos
                  reales)
                </CardDescription>
              </CardHeader>
              <CardContent>
