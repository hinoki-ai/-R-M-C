        ...
        <div className="space-y-3">
          {contacts.map((contact, index) => (
            <div key={index} className="flex items-center justify-between">
              ...
              <Badge
                variant={
                  contact.type === 'emergency' ? 'destructive' : 'default'
                }
              >
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-between p-3 bg-orange-50 rounded border border-orange-200">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Intersección Principal - Hace 1 hora
              </p>
            </div>
...
function EmergencyProtocols() {
  ...
  const protocols =
    useQuery(api.emergency_protocols.getEmergencyProtocols, {}) || [];
  const recordAccess = useMutation(
    api.emergency_protocols.recordProtocolAccess
  );
    ...
    <>
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Guías y procedimientos descargables para situaciones de emergencia
          </CardDescription>
        </CardHeader>
        <CardContent>
          {protocols.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p className="text-sm">
                Los protocolos se cargarán automáticamente
              </p>
            </div>
          ) : (
                    ...
                    <CardContent className="p-4">
                      ...
                      {protocol.downloadCount > 0 && (
                        <p className="text-xs text-muted-foreground mt-2">
                          Descargado {protocol.downloadCount}{' '}
                          {protocol.downloadCount === 1 ? 'vez' : 'veces'}
                        </p>
                      )}
                    </CardContent>
                  </Card>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Registro de incidentes y respuestas anteriores
        </CardDescription>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
...
export default function EmergenciesPage() {
  const { playEmergency, playUI } = useAudioSystem();

  // Play emergency alert when page loads (for demo - in real app this would be based on actual emergencies)
  useEffect(() => {
    // Only play on initial load, not on re-renders
    const hasPlayedWelcome = sessionStorage.getItem('emergency-page-visited');
    if (!hasPlayedWelcome) {
      playEmergency();
      sessionStorage.setItem('emergency-page-visited', 'true');
    }
  }, [playEmergency]);
  ...
  const handleReportIncident = () => {
    playUI('click', 'warning');
    // In real implementation, this would open incident reporting form
    console.log('Incident reporting initiated');
  };
