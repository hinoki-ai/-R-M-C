...
const emergencyProtocols = [
  ...
  },
  {
    ...
    description: 'Medidas preventivas y de respuesta ante situaciones de riesgo',
    steps: [
  ...
  return (
    <div className='space-y-8'>
      ...
      <div className='text-center space-y-4'>
        ...
        >
          <h1 className='text-4xl font-bold tracking-tight'>Información de Emergencias</h1>
          <p className='text-xl text-muted-foreground max-w-3xl mx-auto'>
            Contactos importantes y protocolos de actuación para situaciones de emergencia en Pinto Los Pellines
          </p>
        </motion.div>
      </div>
      ...
      >
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Números importantes para situaciones de emergencia - disponibles 24/7
            </CardDescription>
          </CardHeader>
          <CardContent>
              ...
              {emergencyContacts.map((contact) => (
                <div key={contact.id} className='flex items-center justify-between p-4 border rounded-lg'>
                  <div>
                    ...
                    <p className='text-xs text-muted-foreground'>Disponible: {contact.available}</p>
                  </div>
                  <Badge variant={contact.type === 'police' ? 'destructive' : 'default'}>
                    {contact.type === 'police' ? 'Policía' :
                     contact.type === 'medical' ? 'Médico' :
                     contact.type === 'fire' ? 'Bomberos' :
                     contact.type === 'community' ? 'Comunidad' : 'Seguridad'}
                  </Badge>
                </div>
              ))}
            </div>
                  ...
                  <div className='flex items-start gap-4'>
                    ...
                    <div className='flex-1'>
                      <div className='flex items-center gap-2 mb-2'>
                        ...
                        <Badge variant={protocol.severity === 'high' ? 'destructive' : 'default'}>
                          {protocol.severity === 'high' ? 'Alta Prioridad' : 'Media Prioridad'}
                        </Badge>
                      </div>
      ...
      >
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Información importante para estar preparado ante cualquier eventualidad
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
              <div className='text-center p-4 border rounded-lg'>
                ...
                <h3 className='font-semibold mb-2'>Mantén tu teléfono cargado</h3>
                <p className='text-sm text-muted-foreground'>
                  Asegúrate de tener batería suficiente y los números de emergencia guardados
                </p>
              </div>
              <div className='text-center p-4 border rounded-lg'>
                <div className='text-4xl mb-4'>🏠</div>
              ...
              <div className='text-center p-4 border rounded-lg'>
                ...
                <p className='text-sm text-muted-foreground'>
                  Prepara agua, alimentos no perecederos, medicamentos y documentos importantes
                </p>
              </div>
            </div>
          </CardContent>
...
export default function EmergencyInfoPage() {
  return (
    <DocumentDashboardLayout user={{ id: '', name: 'Usuario', email: '', role: 'user', isAdmin: false }} currentSection='emergency-info'>
