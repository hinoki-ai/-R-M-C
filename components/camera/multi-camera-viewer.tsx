...
import { IconGridDots, IconLayoutGrid, IconMaximize, IconMinimize, IconRefresh } from '@tabler/icons-react'
...
export function MultiCameraViewer({
  ...
  // Fetch camera feeds
  const feeds = useQuery(api.cameras.getCameraFeeds, { cameraId: camera._id }) || []
  ...
  const handleRefresh = () => {
    ...
    onEvent?.('refresh', { cameraId: camera._id, feedsCount: activeFeeds.length })
  }
  ...
  const getGridClassName = (layout: GridLayout) => {
    switch (layout) {
      ...
      case '2x1': return 'grid-cols-1 sm:grid-cols-2'
      case '2x2': return 'grid-cols-2'
      case '3x1': return 'grid-cols-1 sm:grid-cols-3'
      case '3x2': return 'grid-cols-2 sm:grid-cols-3'
      case '4x1': return 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-4'
      case '4x2': return 'grid-cols-2 sm:grid-cols-4'
      case '5x1': return 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-5'
        ...
        <div className='flex items-center justify-between'>
          <CardTitle className='text-lg flex items-center gap-2'>
            ...
            <Badge variant='outline' className='ml-2'>
              {activeFeeds.length} {activeFeeds.length === 1 ? 'Camera' : 'Cameras'}
            </Badge>
          </CardTitle>
          <div className='flex items-center gap-2'>
            <div className={`w-3 h-3 rounded-full ${camera.isActive ? 'bg-green-500' : 'bg-gray-500'}`} />
        ...
        <div className='space-y-2'>
          ...
          {activeFeeds.length > 1 && (
            <div className='flex items-center gap-2 text-sm'>
              ...
              <Select value={layout} onValueChange={(value: GridLayout) => setLayout(value)}>
              ...
              <div className='flex items-center justify-between'>
                <div className='flex items-center gap-2'>
                  ...
                  <div className='text-sm text-gray-600 dark:text-gray-400'>
                    {activeFeeds.length} active feed{activeFeeds.length !== 1 ? 's' : ''}
                  </div>
                </div>
