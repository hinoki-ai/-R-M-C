...
export function EmergencyProtocolForm({ protocol, onSuccess, onCancel }: EmergencyProtocolFormProps) {
  const [formData, setFormData] = useState({
    ...
    emergencyContacts: protocol?.emergencyContacts || [{ name: '', phone: '', role: '' }],
  ...
  const createProtocol = useMutation(api.emergency_protocols.createEmergencyProtocol)
  const updateProtocol = useMutation(api.emergency_protocols.updateEmergencyProtocol)

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()

    if (!formData.title.trim() || !formData.description.trim() || !formData.pdfUrl.trim()) {
    ...
    try {
      if (protocol) {
        ...
        await updateProtocol({
          protocolId: protocol.id as Id<'emergencyProtocols'>,
      ...
      toast.error('Hubo un error al guardar el protocolo. Por favor, inténtalo de nuevo.')
    } finally {
  ...
  const handleFileUpload = async (file: File, type: 'pdf' | 'thumbnail') => {
    ...
    try {
      ...
      toast.success(`${type === 'pdf' ? 'PDF' : 'Miniatura'} subida exitosamente.`)
    } catch (error) {
      toast.error(`Error al subir el ${type === 'pdf' ? 'PDF' : 'archivo de miniatura'}.`)
    } finally {
  ...
  return (
    ...
    >
      ...
      <div className='space-y-4'>
        ...
        <div className='flex items-center space-x-2'>
          <Switch
            ...
            onCheckedChange={(checked) => handleInputChange('offlineAvailable', checked)}
          />
      ...
      {(formData.priority === 'high' || formData.priority === 'critical') && (
        ...
        >
          ...
          <p className='text-xs text-red-700 dark:text-red-300 mt-1'>
            Este protocolo aparecerá destacado y será prioritario en situaciones de emergencia.
          </p>
        </motion.div>
      )}
      ...
      <div className='flex items-center justify-end space-x-3 pt-4 border-t'>
        ...
        <Button
          type='submit'
          disabled={isSubmitting || !formData.title.trim() || !formData.description.trim() || !formData.pdfUrl.trim()}
        >
          {isSubmitting ? 'Guardando...' : protocol ? 'Actualizar Protocolo' : 'Crear Protocolo'}
        </Button>
      </div>
    </motion.form>
  )
