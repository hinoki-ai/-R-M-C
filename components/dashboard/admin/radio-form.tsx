...
export function RadioForm({ station, onSuccess, onCancel }: RadioFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (
      formData.backupStreamUrl &&
      !validateStreamUrl(formData.backupStreamUrl)
    ) {
    ...
    try {
      ...
      toast.error(
        'Hubo un error al guardar la estación de radio. Por favor, inténtalo de nuevo.'
      );
    } finally {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <Select
              ...
              onValueChange={(value: any) => handleInputChange('region', value)}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
