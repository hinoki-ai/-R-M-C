...
interface DashboardErrorBoundaryProps {
  ...
  fallback?: React.ComponentType<{ error: DashboardErrorInfo; onRetry: () => void; onGoHome: () => void }>
  maxRetries?: number
  retryDelay?: number
}
...
function categorizeError(error: Error): DashboardErrorInfo {
  ...
  // Data/query errors
  if (message.includes('query') || message.includes('data') || message.includes('validation')) {
    return {
      type: ErrorType.DATA,
      message: 'Data Error',
...
export class DashboardErrorBoundary extends React.Component<DashboardErrorBoundaryProps, DashboardErrorBoundaryState> {
  ...
  render() {
    if (this.state.hasError && this.state.error) {
      ...
      return <DefaultDashboardErrorFallback
        error={errorInfo}
        onRetry={this.handleRetry}
  ...
  const getErrorIcon = (type: ErrorType) => {
    switch (type) {
      case ErrorType.NETWORK:
        return isOnline ? <IconWifi className='h-8 w-8' /> : <IconWifiOff className='h-8 w-8' />
    ...
    <div className='min-h-[400px] flex items-center justify-center p-4'>
      <Card className='w-full max-w-2xl'>
        <CardHeader className='text-center'>
          <div className={`mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-muted ${getErrorColor(error.type)}`}>
        ...
        <CardContent className='space-y-6'>
          ...
          <div className='flex flex-col sm:flex-row gap-3 justify-center'>
            ...
            {error.type === ErrorType.AUTHENTICATION && (
              <Button onClick={() => router.push('/sign-in')} variant='outline' className='flex items-center gap-2'>
