...
export function ErrorReportingDashboard({ isOpen, onClose }: ErrorReportingDashboardProps) {
  const {
    getLogs,
    getLogsByType,
    getStats,
  ...
  const [selectedType, setSelectedType] = useState<ConvexErrorType | 'all'>('all')
  ...
  const refreshData = () => {
    const allLogs = selectedType === 'all' ? getLogs() : getLogsByType(selectedType)
    const filteredLogs = showResolved ? allLogs : allLogs.filter(log => !log.resolved)
              ...
              <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2'>
                {Object.entries(stats.byType || {}).map(([type, count]) => (
                  <div key={type} className='text-center'>
                    <div className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs ${getErrorTypeColor(type)}`}>
          ...
          <div className='flex items-center justify-between'>
            <div className='flex items-center gap-4'>
              <div className='flex items-center gap-2'>
                <label htmlFor='error-type-filter' className='text-sm font-medium'>Filter by type:</label>
                <select
                  id='error-type-filter'
                  value={selectedType}
                  onChange={(e) => setSelectedType(e.target.value as ConvexErrorType | 'all')}
              ...
              >
                {showResolved ? <IconEyeOff className='h-4 w-4' /> : <IconEye className='h-4 w-4' />}
                    ...
                    logs.map((log) => (
                      <div key={log.id} className='border rounded-lg p-4'>
                        <div className='flex items-start justify-between mb-2'>
                          <div className='flex items-center gap-2'>
                            <span className='text-lg'>{getErrorTypeIcon(log.type)}</span>
                            <Badge variant={log.resolved ? 'secondary' : 'destructive'} className='capitalize'>
                              {log.type}
                            </Badge>
                            {log.resolved && (
