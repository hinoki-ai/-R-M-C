...
const RssNewsSection: React.FC = () => {
  ...
  const articles =
    useQuery(api.rss.getRssArticles, {
      category:
          ...
          <TabsContent key={category.id} value={category.id} className="mt-6">
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center space-x-2 mb-2">
                          <Badge
                            variant="outline"
                            className={`text-xs ${getCategoryColor(article.category)} text-white`}
                          >
                            {article.category}
                          </Badge>
                          <Badge
                            variant="secondary"
                            className={`text-xs ${getRegionColor(article.region)} text-white`}
                          >
                            {article.region}
                          </Badge>
                    ...
                    <div className="flex items-center justify-between mt-4 pt-4 border-t">
                      <div className="flex items-center space-x-2">
                        <Button
                          ...
                          size="sm"
                          onClick={() =>
                            handleMarkAsRead(article._id, !article.isRead)
                          }
                          className="h-8"
                        >
                          {article.isRead ? (
            ...
            {articles.length === 0 && (
              <div className="text-center py-12">
                ...
                <p className="text-muted-foreground">
                  Los artículos se actualizarán automáticamente desde las
                  fuentes RSS.
                </p>
              </div>
            )}
