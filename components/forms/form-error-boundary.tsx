  ...
  return (
    <Card className='border-destructive/50'>
      <CardHeader className='pb-3'>
        ...
        <CardDescription>
          Ocurrió un problema al procesar el formulario. Tus datos están seguros.
        </CardDescription>
      </CardHeader>
...
export function useFormErrorHandler(formName?: string) {
  const { captureError, resetError, hasError, errorId } = useComponentErrorBoundary(`Form-${formName || 'unknown'}`)

  const handleFormError = React.useCallback((error: Error | string) => {
