...
import { requireAuth, requireUser } from './utils/error_handler';
...
export const getUpcomingEvents = query({
  ...
  handler: async (ctx, args) => {
    try {
      ...
      const allEventIds = [
        ...new Set([...eventIds, ...organizedEvents.map(e => e._id)]),
      ];
      ...
      for (const eventId of allEventIds) {
        ...
        if (eventDateTime > now && eventDateTime <= futureDate) {
          ...
          const userAttendance = attendeeEvents.find(
          ...
          upcomingEvents.push({
            ...
            userAttendanceStatus:
              ...
              (event.organizerId === user._id ? 'organizer' : null),
          });
        }
      }
      ...
      // Return empty array if not authenticated or user not found
      return [];
    }
  },
});
...
export const sendEventReminders = action({
  ...
  handler: async ctx => {
    ...
    await ctx.runMutation(
      api.calendar_notifications.sendEventRemindersMutation,
      {}
    );
  },
});
  ...
  handler: async ctx => {
    ...
    for (const reminder of reminders) {
      ...
      const reminderTime = new Date(
        eventDateTime.getTime() - reminder.reminderTime * 60 * 1000
      );
...
export const createEventReminder = action({
  ...
  handler: async (ctx, args): Promise<Id<'eventReminders'>> => {
    return await ctx.runMutation(
      api.calendar_notifications.createEventReminderMutation,
      args
    );
  },
});
