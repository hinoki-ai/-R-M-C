...
export const getAnnouncements = query({
  ...
  handler: withErrorHandling(async (ctx, args: {}) => {
    return await ctx.db
      .query('announcements')
      .withIndex('byActive', (q: any) => q.eq('isActive', true))
      .order('desc')
      .take(10);
  }, 'getAnnouncements'),
});
...
export const markAnnouncementAsRead = mutation({
  ...
  handler: withErrorHandling(
    async (
      ...
      const announcement = await safeDbGet(
        ctx,
        args.announcementId,
        'announcement'
      );
      if (!announcement) throw createNotFoundError('Announcement');
...
export const updateAnnouncement = mutation({
  ...
  handler: withErrorHandling(
    async (
      ...
      if (userRole !== 'admin') {
        throw createAuthzError('Only administrators can update announcements');
      }
...
export const deleteAnnouncement = mutation({
  ...
  handler: withErrorHandling(
    async (
      ...
      const userRole = user.role || 'user';
      if (userRole !== 'admin') {
        throw createAuthzError('Only administrators can delete announcements');
      }
...
export const getMaintenanceRequests = query({
  args: {},
  handler: async ctx => {
    return await ctx.db.query('maintenanceRequests').order('desc').take(10);
  },
});
...
export const getDashboardStats = query({
  ...
  handler: async ctx => {
    ...
    const contributionAmount = totalContributions.reduce(
...
export const updatePaymentStatus = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const payment = await ctx.db
      .query('payments')
      .withIndex('byReferenceId', q =>
        q.eq('referenceId', args.paymentIntentId)
      )
      .first();
...
export const getMemberRankings = query({
  ...
  handler: async ctx => {
    try {
      ...
      const [contributionPayments, projectContributions, paymentAttempts] =
        await Promise.all([
          ctx.db
            .query('payments')
            .filter(q =>
      ...
      for (const payment of contributionPayments) {
        if (payment.amount > 0 && payment.amount < 10000000) {
      ...
      for (const contribution of projectContributions) {
        if (contribution.amount > 0 && contribution.amount < 10000000) {
          // Reasonable upper limit
      ...
      for (const attempt of paymentAttempts) {
        ...
        const userMonths =
          userSubscriptions.get(attempt.userId) || new Set<string>();
      ...
      for (const user of users) {
        ...
        const monthlyData =
          userSubscriptions.get(user._id) || new Set<string>();
  ...
  handler: async ctx => {
    try {
      ...
      recentAnnouncements.forEach(announcement => {
        ...
        engagementByDay[dayOfWeek].interactions +=
