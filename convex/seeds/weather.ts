...
export const seedWeather = mutation({
  ...
  handler: async (ctx) => {
    ...
    console.log(`📊 Existing weather data points in database: ${existingWeatherData.length}`);
    console.log(`📊 Existing weather alerts in database: ${existingAlerts.length}`);
    console.log(`📊 Existing weather forecasts in database: ${existingForecasts.length}`);
