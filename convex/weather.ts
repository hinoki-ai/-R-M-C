...
export const updateWeatherData = mutation({
  args: {
    ...
    updates: v.object({
      ...
      location: v.optional(v.string()),
      source: v.optional(
        v.union(v.literal('api'), v.literal('manual'), v.literal('sensor'))
      ),
      isHistorical: v.optional(v.boolean()),
    }),
  },
...
// Get all weather alerts for admin (includes inactive ones)
export const getAllWeatherAlerts = query({
  args: {},
  returns: v.array(
    v.object({
      _id: v.id('weatherAlerts'),
  ...
  handler: async ctx => {
    return await ctx.db.query('weatherAlerts').collect();
  },
});
...
export const getWeatherForecasts = query({
  ...
  handler: async (ctx, args) => {
    ...
    const forecasts = await query.order('asc').take(days);
