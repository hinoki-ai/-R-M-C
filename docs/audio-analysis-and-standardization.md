...
const useAudioSystem = () => {
  ...
  return {
    playUI: (action: string) => { /* UI sounds */ },
    playVoice: (category: string) => { /* Voice announcements */ },
    playAlert: (type: string) => { /* Emergency alerts */ },
    setPreferences: (prefs: Partial<AudioPreferences>) => { /* Update settings */ }
  };
};
```
