...
export const useCommunityVoice = () => {
  ...
  const playCommunityVoice = useCallback((voiceType: string = 'welcome') => {
  ...
  const playRandomVoice = useCallback(() => {
    ...
    const voiceTypes = ['welcome', 'intro', 'meeting', 'event', 'maintenance', 'announce.general', 'weather', 'status', 'calendar', 'service']
    const randomType = voiceTypes[Math.floor(Math.random() * voiceTypes.length)]
