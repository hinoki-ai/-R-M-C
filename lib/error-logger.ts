import { ConvexErrorType, categorizeConvexError } from '@/hooks/use-convex-error-handler';
...
class ErrorLogger {
  ...
  getStats() {
    ...
    const typeCounts = this.logs.reduce(
      (acc, log) => {
  ...
  logConvexError(error: any, additionalData?: Record<string, any>) {
    const categorizedError = categorizeConvexError(error);
  ...
  private async processReportBatch(force = false): Promise<void> {
    ...
    const batchSize = force
      ...
      : this.reportingConfig.batchSize;
