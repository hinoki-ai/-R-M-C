...
class OfflineManager {
  ...
  private saveQueuedRequests() {
    ...
    try {
      localStorage.setItem('offline-queue', JSON.stringify(this.queuedRequests))
    } catch (error) {
  ...
  private async processQueuedRequests() {
    if (this.syncInProgress || !this.isOnline || this.queuedRequests.length === 0) {
      return
    }
    ...
    for (const request of requestsToProcess) {
      try {
        ...
        if (result.success) {
          ...
          console.log(`Successfully processed queued request: ${request.operation}`)
        } else {
          request.retryCount++
          if (request.retryCount >= request.maxRetries) {
            console.warn(`Max retries exceeded for queued request: ${request.operation}`)
            this.removeRequest(request.id)
          } else {
            this.saveQueuedRequests()
          }
        ...
        request.retryCount++
        if (request.retryCount >= request.maxRetries) {
          console.error(`Failed to process queued request after max retries: ${request.operation}`, error)
          this.removeRequest(request.id)
        } else {
          this.saveQueuedRequests()
        }
  ...
  private async executeQueuedRequest(request: OfflineRequest): Promise<{ success: boolean }> {
    // This would need to be implemented based on the specific operations
    // For now, we'll just simulate success
    console.log(`Processing queued request: ${request.operation}`, request.args)
...
export async function offlineAwareFetch<T>(
  ...
  if (!manager.getState().isOnline) {
    ...
    throw new ConvexError({
      type: 'network',
      message: 'You are currently offline. Your request has been queued and will be processed when you are back online.',
      details: { queued: true },
      retryable: false
    })
  }
