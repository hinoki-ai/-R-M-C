...
export const getNavigationConfig = (user: User) => {
  ...
  return {
    primary: filteredSections.filter(section => (NAVIGATION_GROUPS.primary as readonly string[]).includes(section.id)),
    secondary: filteredSections.filter(section => (NAVIGATION_GROUPS.secondary as readonly string[]).includes(section.id)),
    system: filteredSections.filter(section => (NAVIGATION_GROUPS.system as readonly string[]).includes(section.id)),
...
export const getSectionPriority = (sectionId: string): 'high' | 'medium' | 'low' => {
  const section = DASHBOARD_SECTIONS.find(s => s.id === sectionId)
...
export const searchSections = (query: string, user: User): DashboardSection[] => {
...
export const filterSectionsByPriority = (sections: DashboardSection[], priority: 'high' | 'medium' | 'low'): DashboardSection[] => {
  return sections.filter(section => section.priority === priority)
}

export const sortSectionsByPriority = (sections: DashboardSection[]): DashboardSection[] => {
  const priorityOrder = { high: 3, medium: 2, low: 1 }
  return sections.sort((a, b) => priorityOrder[b.priority || 'low'] - priorityOrder[a.priority || 'low'])
}

// Accessibility Helpers
export const getSectionAriaLabel = (sectionId: string): string => {
  const section = DASHBOARD_SECTIONS.find(s => s.id === sectionId)
  return section ? `${section.label} - ${section.description}` : 'Dashboard Section'
}
...
export const getQuickActions = (user: User) => {
  const baseActions = [
    {
      id: 'report-emergency',
...
// Helper function for navigation (needs to be defined)
const navigateTo = (sectionId: string) => {
  const section = DASHBOARD_SECTIONS.find(s => s.id === sectionId)
  if (section) {
    // This would normally use router.push, but we define it here for the config
    console.log(`Navigate to: ${section.url}`)
  }
}