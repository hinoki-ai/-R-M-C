...
try {
  const capacitorConfigContent = fs.readFileSync(path.join(__dirname, '../capacitor.config.ts'), 'utf8');
  ...
  tests.capacitor.checks.push({
    ...
    actual: capacitorConfigContent.includes("appId: 'com.juntadevecinos.app'") ? 'Present' : 'Missing'
  });
  ...
  tests.capacitor.checks.push({
    ...
    actual: capacitorConfigContent.includes("appName: 'Pinto Los Pellines'") ? 'Present' : 'Missing'
  });
  ...
  tests.capacitor.checks.push({
    ...
    expected: 'false',
    actual: capacitorConfigContent.includes('bundledWebRuntime: false') ? 'Present' : 'Missing'
  });
  ...
  tests.capacitor.checks.push({
    ...
    actual: capacitorConfigContent.includes("scheme: 'pintopellines'") ? 'Present' : 'Missing'
  });
} catch (error) {
...
try {
  ...
  tests.security.checks.push({
    ...
    expected: 'Present',
    actual: nextConfigContent.includes('Content-Security-Policy') ? 'Present' : 'Missing'
  });
  ...
  tests.security.checks.push({
    ...
    expected: 'Present',
    actual: nextConfigContent.includes('Strict-Transport-Security') ? 'Present' : 'Missing'
  });
  ...
  tests.security.checks.push({
    ...
    expected: 'Present',
    actual: nextConfigContent.includes('X-Frame-Options') ? 'Present' : 'Missing'
  });
} catch (error) {
...
try {
  ...
  tests.pwa.checks.push({
    name: 'Manifest Name',
    status: manifest.name === 'Pinto Los Pellines - Gestión Comunitaria' ? '✅' : '❌',
...
try {
  ...
  tests.pwa.checks.push({
    ...
    status: swContent.includes('pellines-mobile-v1') ? '✅' : '❌',
    expected: 'Cache implementation present',
    actual: swContent.includes('pellines-mobile-v1') ? 'Present' : 'Missing'
  });
  ...
  tests.pwa.checks.push({
    ...
    actual: swContent.includes('self.addEventListener(\'push\'') ? 'Present' : 'Missing'
  });
} catch (error) {
...
try {
  const componentContent = fs.readFileSync(path.join(__dirname, '../components/mobile-app-download.tsx'), 'utf8');

  tests.accessibility.checks.push({
    name: 'ARIA Labels',
    status: componentContent.includes('aria-labelledby') && componentContent.includes('aria-label') ? '✅' : '❌',
    expected: 'ARIA attributes present',
    actual: componentContent.includes('aria-labelledby') && componentContent.includes('aria-label') ? 'Present' : 'Missing'
  });

  tests.accessibility.checks.push({
    name: 'Keyboard Navigation',
    status: componentContent.includes('onKeyDown') && componentContent.includes('tabIndex') ? '✅' : '❌',
    expected: 'Keyboard navigation implemented',
    actual: componentContent.includes('onKeyDown') && componentContent.includes('tabIndex') ? 'Present' : 'Missing'
  });
  ...
  tests.performance.checks.push({
    ...
    actual: componentContent.includes('DownloadSkeleton') ? 'Present' : 'Missing'
  });
  ...
  tests.performance.checks.push({
    ...
    actual: componentContent.includes('PerformanceObserver') ? 'Present' : 'Missing'
  });

  tests.accessibility.checks.push({
    name: 'Error Handling',
    status: componentContent.includes('try') && componentContent.includes('catch') ? '✅' : '❌',
    expected: 'Error handling implemented',
    actual: componentContent.includes('try') && componentContent.includes('catch') ? 'Present' : 'Missing'
  });
  ...
  tests.accessibility.checks.push({
    ...
    actual: componentContent.includes('Seguridad Garantizada') ? 'Present' : 'Missing'
  });
} catch (error) {
...
// Overall Score
const totalChecks = Object.values(tests).reduce((sum, category) => sum + category.checks.length, 0);
const passedChecks = Object.values(tests).reduce((sum, category) =>
  sum + category.checks.filter(check => check.status === '✅').length, 0
);
